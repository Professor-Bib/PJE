	...
	app.controller('JenisPengirimanListController', [ '$scope', '$location',
			...
			function($scope, $location, $dialogs, JenisPengirimanListFactory,JenisPengirimanEditFactory) {
			   ...
			   $scope.remove = function (jenisPengiriman) {
		        	dlg = $dialogs.confirm('Konfirmasi','Apakah anda ingin hapus Jenis Pengiriman ini : ' + jenisPengiriman.nama_service04262564);
		        	dlg.result.then(function(btn){
		        		JenisPengirimanEditFactory.remove({id: jenisPengiriman.id_kurir_service04262564}, function(data){
			    ...
			    $scope.edit = function (jenisPengiriman) {
		            $location.path('/master/jenis-pengiriman/' + jenisPengiriman.id_kurir_service04262564 + '/edit');
		        };
		        
		        $scope.detail = function (jenisPengiriman) {
		            $location.path('/master/jenis-pengiriman/' + jenisPengiriman.id_kurir_service04262564 + '/detail');
		        };
        ...
        app.controller('JenisPengirimanCreateController', [
			...
			function($scope, $routeParams, $location,$dialogs,
				...
				$scope.title = "Buat Baru Jenis Pengiriman";
				$scope.isEdit = false;
...
//                                ];
				...
				$scope.save = function() {
					JenisPengirimanEditFactory.update({
						id : $scope.jenisPengiriman.id_kurir_service04262564,
						jenisPengiriman : $scope.jenisPengiriman
					}, function(data) {
						notif($dialogs, data.status, 'Simpan');
						$location.path('/master/jenis-pengiriman');
